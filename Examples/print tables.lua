...
--------------------------------------------------------------------------------
--[[
  PrintMxmlAsLua function is a tool for printing the exml-2-lua data.
  Paste one or more sections (must be complete sections) or full exml files,
  run the string through PrintMxmlAsLua, then print to output or save to file.
]]
...
local function GetTableFromFile(path)
	...
	if f then
		local s = PrintMxmlAsLua({exml=f:read('*a'), indent='  '})
		f:close()
	end
	return s
end
...
local function GetTableFromString(s)
	return PrintMxmlAsLua({exml=s, com='"'})
end
